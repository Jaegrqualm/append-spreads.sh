...
do
	...
	convert $(printf "%02g" "$a").png $i.png -strip +append dump/$i-$(printf "%02g" "$a").png # printf re-adds the zero padding to the other filename number. -strip removes png metadata(?) that causes consecutive runs on the same files to produce results with different checksums.
done
